...
function RichTextEditor({
  ...
  const getYouTubeId = (u: string) => { const m = u.match(/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/|live\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/); return m ? m[1] : null; }; const getVimeoId = (u: string) => { const m = u.match(/vimeo\.com\/(?:video\/)?(\d+)/); return m ? m[1] : null; }; const insertVideo = () => { const url = prompt('URL del video (YouTube, Vimeo, Instagram, TikTok o Facebook):'); if (!url) return; const trimmed = url.trim(); let embedHtml = ''; const ytId = getYouTubeId(trimmed); const vimeoId = getVimeoId(trimmed); if (ytId) { embedHtml = `<div><iframe src="https://www.youtube.com/embed/${ytId}" frameborder="0" allowfullscreen style="width:100%;max-width:560px;height:315px;"></iframe></div>`; } else if (vimeoId) { embedHtml = `<div><iframe src="https://player.vimeo.com/video/${vimeoId}" frameborder="0" allowfullscreen style="width:100%;max-width:560px;height:315px;"></iframe></div>`; } else if (/instagram\.com\/(reel|p)\//.test(trimmed)) { embedHtml = `<blockquote class="instagram-media" data-instgrm-permalink="${trimmed}" style="width:100%;max-width:540px;margin:0 auto;"></blockquote>`; const script = document.createElement('script'); script.src = 'https://www.instagram.com/embed.js'; script.async = true; document.body.appendChild(script); } else if (/tiktok\.com\/.+\/video\//.test(trimmed)) { embedHtml = `<blockquote class="tiktok-embed" cite="${trimmed}" style="max-width:605px;min-width:325px;margin:0 auto;"><section></section></blockquote>`; const script = document.createElement('script'); script.src = 'https://www.tiktok.com/embed.js'; script.async = true; document.body.appendChild(script); } else if (/facebook\.com\/.+\/videos\//.test(trimmed)) { const encoded = encodeURIComponent(trimmed); embedHtml = `<div><iframe src="https://www.facebook.com/plugins/video.php?href=${encoded}&show_text=false" frameborder="0" allowfullscreen style="width:100%;max-width:560px;height:315px;"></iframe></div>`; } else { alert('No pude reconocer un video de YouTube, Vimeo, Instagram, TikTok o Facebook en ese link. La mayoria de otras paginas no permiten insertarse dentro de un iframe.'); return; } document.execCommand('insertHTML', false, embedHtml); editorRef.current?.focus(); onChange(editorRef.current?.innerHTML || ''); };
